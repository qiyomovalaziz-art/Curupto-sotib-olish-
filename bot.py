...
async def start(msg: types.Message):
    ...
    await msg.answer("Bo‘lim tanlang 👇", reply_markup=menu)

@dp.callback_query_handler(lambda c: c.data == "check_sub")
async def retry(call):
    await start(call.message)

# ===== RASM/VIDEO/FILE KO‘RSATISH (NEXT/BACK BILAN) =====
async def show_item(msg, items, index, cat):
    ...
    media = (
        InputMediaPhoto(file_id, caption=caption) if cat == "rasm" else
        InputMediaDocument(file_id, caption=caption) if cat == "fayl" else
    ...
    try:
        ...
        if cat == "rasm":
            ...
            await msg.answer_document(file_id, caption=caption, reply_markup=kb)
        else:
            await msg.answer_video(file_id, caption=caption, reply_markup=kb)

@dp.callback_query_handler(lambda c: c.data in ["rasm","fayl","video"])
async def open_category(call):
    cat = call.data
    sql.execute("SELECT id FROM files WHERE kategoriya=?", (cat,))
    items = [i[0] for i in sql.fetchall()]
    if not items:
        return await call.message.answer("📭 Bu bo‘limda fayl yo‘q.")
...
async def admin(call):
    ...
    kb.add(
        ...
        InlineKeyboardButton("➕ Fayl qo‘shish", callback_data="add_fayl"),
        InlineKeyboardButton("➕ Video qo‘shish", callback_data="add_vid")
    )
...
async def ask_channel(call):
    await call.message.answer("📡 Kanal username kiriting (masalan: @Kanal):")
