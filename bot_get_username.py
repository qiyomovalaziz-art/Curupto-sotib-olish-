...
def group_message_handler(update: Update, context: CallbackContext):
    ...
    # hech qanday javob bermaymiz, faqat saqlaymiz
    return
...
def get_user_handler(update: Update, context: CallbackContext):
    ...
    if not context.args:
        update.message.reply_text("Foydalanish: /get <user_id yoki @username>\nEslatma: faqat botning o'zi saqlagan ma'lumotlar ko'rsatiladi.")
        return

    query = context.args[0].strip()
    # avval numeric ID bilan tekshir
    uentry = data["users"].get(str(query))
    if uentry is None:
        # agar @username yoki username kiritilgan bo'lsa, qidiruv qilamiz
        uname = query[1:] if query.startswith("@") else query
        found = None
        for k, v in data["users"].items():
            if v.get("username") and v["username"].lower() == uname.lower():
                found = (k, v)
                break
        if found:
            uid, uentry = found
    ...
    for cid in seen_chats:
        ...
        if chat_info:
            ...
            chat_lines.append(cid)

    text = (
        f"<b>Ma'lumot (saqlangan):</b>\n"
        f"ID: <code>{html.escape(str(query))}</code>\n"
...
def main():
    ...
    dp.add_handler(MessageHandler(~Filters.private, group_message_handler))               # private bo'lmagan xabarlarni qayd etadi (guruh/channel)
    dp.add_handler(MessageHandler(Filters.status_update.new_chat_members, new_members_handler))  # yangi a'zo qo'shilganda
